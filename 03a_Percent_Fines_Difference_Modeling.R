...
####******** MODELING DIFFERENCE for PERCENT FINES ********************######
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##
...
# klamath pred_obs data for testing on all post-fire sites -- produced from Percent_Fines_Unburned_Modeling script
pred_obs.pf.k <- read_csv("RF_models/FINAL_RF_MODELS/percent_fines/post_fire_testing/predicted_observed/klamath_perc_fines_post_fire_pred_obs.csv")

pred_obs.pf.c <- read_csv("RF_models/FINAL_RF_MODELS/percent_fines/post_fire_testing/predicted_observed/cascades_perc_fines_post_fire_pred_obs.csv")
...
for (model_run in names(model_data_list)) {
  ...
  rf_model <- ranger(difference~ . , data= model_data , 
                     num.trees =625, mtry = 1, min.node.size = 6,
...
### Individual Partial Dependence Plots of Interest

## Soil Organic Matter
omws.kd <- partial(rf.kd, pred.var = c("OMWS"),
                   train = diff_filter, prob = TRUE)

ggplot(omws.kd, aes(x = OMWS, y = yhat))+
  geom_line()+
  geom_point()+
  labs(title = "PDP OMWS (Klamath)",
       x = "OMWS",
       y = "predicted response")

## Rock Strength
comp.kd <- partial(rf.kd, pred.var = c("COMPSTRGTHWS"),
                   train = diff_filter, prob = TRUE)

ggplot(comp.kd, aes(x = COMPSTRGTHWS, y = yhat))+
  geom_line()+
  geom_point()+
  labs(title = "PDP COMPSTRGTHWS (Klamath)",
       x = "COMPSTRGTHWS",
       y = "predicted response")

## Baseflow Index
bfi.kd <- partial(rf.kd, pred.var = c("BFIWS"),
                   train = diff_filter, prob = TRUE)

ggplot(bfi.kd, aes(x = BFIWS, y = yhat))+
  geom_line()+
  geom_point()+
  labs(title = "PDP BFIWS (Klamath)",
       x = "BFIWS",
       y = "predicted response")
...
for (model_run in names(model_data_list3)) {
  ...
  # Fit a regression random forest model to predict the difference
  rf_model <- ranger(difference~ . , data= model_data , 
                     num.trees =875, mtry = 12, min.node.size = 2,
                     importance = 'permutation', 
                     scale.permutation.importance = TRUE) 
...
#### Partial Dependence Plots for Cascades ####

# Choose variable of interest
target_variable.c <- "mean_cc"
...
ggplot(pdp_combined.c, aes_string(x = target_variable.c, y = "y", group = "interaction(LineType, model_run)", linetype = "LineType")) +
  ...
  scale_fill_manual(name="", values= scale)+
   theme(legend.position = c(0.1, 0.95),  # Move legend box to upper right
...
#### Partial Dependence Plots for Klamath ####

# Choose variable of interest
target_variable <- "OMWS"
...
ggplot(pdp_combined, aes_string(x = target_variable, y = "y", group = "interaction(LineType, model_run)", linetype = "LineType")) +
  ...
  theme_classic() +
  scale_fill_manual(name="", values= scale)+
   theme(legend.position = c(0.85, 0.15),  # Move legend box to upper right
         text = element_text(size = 25),            
         legend.background = element_rect(fill = "white", color = "black"))
