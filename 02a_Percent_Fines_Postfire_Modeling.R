...
pf_data.k <- pf_survey_covars %>% 
  ...
  dplyr::select(c(perc_fines, ppc_burned_sqkm,
...
for(n in 1:50){
  set.seed(n)
  rf <- ranger(perc_fines~ . , data=pf_data.k, 
               num.trees =700, mtry = 4, min.node.size = 7,
  ...
  predicted <- data.frame(pred = rf$predictions, obs = pf_data.k$perc_fines, 
...
rf.k <- ranger(perc_fines~ . , data=pf_data.k, 
               num.trees =700, mtry = 4, min.node.size = 7,
               importance = 'permutation', 
               scale.permutation.importance = TRUE) 
...
klam <- ggplot(importance_data.k, aes(x= importance, y = variable, fill = category)) +  
 ...
 # ggtitle("Klamath Postfire %Fines Model")+
  inset_element(pdp_plot.k, left = .01, bottom = -0.03 , right = 0.22, top = 1)
...
for(n in 1:50){
  set.seed(n)
  rf <- ranger(perc_fines~ . , data=pf_data.c, 
  ...
  predicted <- data.frame(pred = rf$predictions, obs = pf_data.c$perc_fines,
...
#### CREATE PARTIAL DEPENDENCE PLOTS for Variable Importance Figure ####
# build one RF model to use for the PDPs
rf.c <- ranger(perc_fines~ . , data=pf_data.c, 
               num.trees =812, mtry = 10, min.node.size = 3,
               importance = 'permutation', 
               scale.permutation.importance = TRUE) 
...
casc <- ggplot(importance_data.c, aes(x= importance, y = variable, fill = category)) +  
  ...
  scale_x_continuous(limits = c(0, 44),
                     breaks = seq(0,44, by = 10))+
  inset_element(pdp_plot.c, left = -.002, bottom = -0.03 , right = 0.23, top = 1)
...
output_dir.k <- "FIGURES/supplement/PDP/postfire_klam_perc_fines"
dir.create(output_dir.k, showWarnings = FALSE)

# Define your ten target variables
target_variables.k <- c("survey_year", "OMWS","COMPSTRGTHWS","relative_bankfull",
                        "PRECIP8110WS", "BFIWS","PERMWS","mean_cc", 
                        "slope23d_perc_greater", "RDDENSWS")
...
for (var in target_variables.k) {
  ...
  for (i in seq_along(model_list.k)) {
    rf_model <- ranger(perc_fines ~ ., data = pf_data.k,
...
# Create a directory to save the plots
output_dir <- "FIGURES/supplement/PDP/postfire_casc_perc_fines"
dir.create(output_dir, showWarnings = FALSE)

# Define your ten target variables
target_variables <- c("bankfull_mean", "PERMWS","PRECIP8110WS","relative_bankfull", 
                      "mean_cc", "ppc_himod_sqkm", "RCKDEPWS",
                      "ppc_burned_sqkm", "area_sqkm.x", "KFFACTWS")
...
for (var in target_variables) {
  ...
  for (i in seq_along(model_list.c)) {
    rf_model <- ranger(perc_fines ~ ., data = pf_data.c,
                       num.trees = 875, mtry = 7, min.node.size = 2,
                       importance = 'permutation', scale.permutation.importance = TRUE)
